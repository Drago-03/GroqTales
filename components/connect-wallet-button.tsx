...
export function ConnectWalletButton() {
  const { address, isConnected, isSigningIn, connect, signIn, connectors } =
    useWeb3Auth();
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  const [availableConnectors, setAvailableConnectors] = useState<
    Record<string, { index: number; name: string; id: string }>
  >({});
  ...
  useEffect(() => {
    if (connectors.length > 0) {
      ...
      const detectedConnectors: Record<
        string,
        { index: number; name: string; id: string }
      > = {};

      // Look for MetaMask or injected providers
      const metaMaskIdx = connectors.findIndex(
        (c) => c.id === 'injected' || c.name.toLowerCase().includes('metamask')
      );
      if (metaMaskIdx >= 0) {
        detectedConnectors['metaMask'] = {
          index: metaMaskIdx,
          name: connectors[metaMaskIdx].name,
          id: connectors[metaMaskIdx].id,
        };
      }
      // Look for WalletConnect
      const walletConnectIdx = connectors.findIndex(
        (c) =>
          c.id === 'walletConnect' ||
          c.name.toLowerCase().includes('walletconnect')
      );
      if (walletConnectIdx >= 0) {
        detectedConnectors['walletConnect'] = {
          index: walletConnectIdx,
          name: connectors[walletConnectIdx].name,
          id: connectors[walletConnectIdx].id,
        };
      }
      // Look for Ledger
      const ledgerIdx = connectors.findIndex(
        (c) => c.id === 'ledger' || c.name.toLowerCase().includes('ledger')
      );
      if (ledgerIdx >= 0) {
        detectedConnectors['ledger'] = {
          index: ledgerIdx,
          name: connectors[ledgerIdx].name,
          id: connectors[ledgerIdx].id,
        };
      }
    ...
    try {
      if (!isConnected) {
        if (connectors.length === 0 || !availableConnectors[connectorKey]) {
          throw new Error(
            `The ${connectorKey === 'metaMask' ? 'MetaMask' : connectorKey === 'walletConnect' ? 'WalletConnect' : 'Ledger'} connector is not available. Please ensure the provider is installed or enabled.`
          );
        }
      ...
      toast({
        ...
        description:
          error.message ||
          `Failed to connect with ${connectorKey === 'metaMask' ? 'MetaMask' : connectorKey === 'walletConnect' ? 'WalletConnect' : 'Ledger'}. Please try again.`,
      });
    }
  };
        ...
        <div className="flex flex-col gap-4 pt-4">
          ...
          <div className="flex flex-col items-center gap-2 border-t pt-4">
            <p className="text-xs text-muted-foreground/70 text-center max-w-xs">
              Note: Wallet options are disabled if the provider is not detected.
              Ensure the extension or app is installed.
            </p>
          </div>
        </div>
      </DialogContent>
