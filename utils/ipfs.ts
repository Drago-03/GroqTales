 ...
 */
export const uploadToIPFS = async (file: File | string | object, metadata?: Record<string, any>): Promise<string> => {
  ...
  if (!PINATA_API_KEY || !PINATA_API_SECRET) {
    console.warn('Using mock IPFS upload as Pinata API credentials are not set');
 ...
 */
const uploadJSONToIPFS = async (jsonData: object, metadata?: Record<string, any>): Promise<string> => {
  try {
    const pinataBody = {
    ...
    if (response.data && response.data.IpfsHash) {
      console.log('Successfully uploaded JSON to IPFS:', response.data.IpfsHash);
      return response.data.IpfsHash;
    } else {
 ...
 */
const mockUploadToIPFS = async (file: File | string | object, metadata?: Record<string, any>): Promise<string> => {
