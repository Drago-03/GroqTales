...
export function Identity({ children, className, hasCopyAddressOnClick }: { children: ReactNode; className?: string; hasCopyAddressOnClick?: boolean }) {
...
export function Avatar({ className }: { className?: string }) {
  return (
    <div className={`w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-sm ${className || ''}`}>
      DU
    </div>
  );
}
