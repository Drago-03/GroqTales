...
function checkEnvVariables() {
  ...
  if (!fs.existsSync(envPath)) {
    console.error('ERROR: .env.local file not found. Please create it with necessary variables.');
    process.exit(1);
  }
  ...
  requiredVars.forEach(varName => {
    if (!envContent.includes(varName)) {
      console.warn(`WARNING: ${varName} not found in .env.local. This may cause issues in production.`);
    } else if (envContent.includes(`${varName}=your_`) || envContent.includes(`${varName}=YOUR_`)) {
      console.warn(`WARNING: ${varName} seems to be using a placeholder value. Replace with a production key.`);
    } else {
...
function checkBuild() {
  ...
  try {
    ...
    console.error('ERROR: Build failed. Please fix build errors before deploying.');
    process.exit(1);
  }
}
